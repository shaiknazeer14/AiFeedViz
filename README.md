...
FeedViz/
    feedviz/
        agents/
            data_processing_agent.py    ← Data loading and cleaning agent
            nlp_analysis_agent.py       ← Sentiment and NLP agent
            scoring_agent.py            ← Performance scoring agent
            ranking_agent.py            ← Teacher ranking agent
            insight_agent.py            ← CrewAI insight generation agent
            retrieval_agent.py          ← CrewAI RAG retrieval agent
        tools/
            text_cleaner.py             ← NLP preprocessing pipeline
            nlp_analyzer.py             ← Sentiment, keywords, topic clustering
            embeddings.py               ← FAISS index build and search
            rag_pipeline.py             ← RAG query and context building
            scorer.py                   ← Weighted scoring formula
            ranker.py                   ← Department and global ranking
            insight_generator.py        ← Teacher insight generation
        dashboard/
...
bashpython feedviz/dashboard/gradio_app.py
...
student_id, teacher_name, department, subject, section, rating, feedback_text, date
